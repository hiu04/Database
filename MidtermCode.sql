...
select extract(year from bs.sale_date) as year,
       ...
       dense_rank() over (order by count(bs.ticket_number)) as number_order_rank
from assignment01.bakery_sales as bs
...
select extract(year from bs.sale_date) as year,
       ...
       rank() over (partition by extract(year from bs.sale_date),
       extract(month from bs.sale_date)
